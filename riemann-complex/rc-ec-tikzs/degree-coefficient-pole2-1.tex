...
\begin{document}
	\begin{tikzpicture}[>=Latex, line cap=round, line join=round, font=\small]
		...
		\begin{scope}[shift={(0,0)}]
			...
			\node[red] at (-1.75,-1.6) {pole of order $-2$};
			...
			\node[align=left] at (0,-2.7) {$\displaystyle
				...
				=\frac{1}{2\pi i}\!\oint_C \frac{-2\,dz}{\,z\,}=-2.$};
		\end{scope}
		...
		\begin{scope}[shift={(7.2,0)}]
			...
			% image curve f(C): z = 1.5 e^{it} -> w = (1/1.5^2) e^{-i 2t}
			\draw[violet,thick,
			postaction={decorate},
			decoration={markings,
			...
			({ (1/2.25)*cos(2*\x r) },{ -(1/2.25)*sin(2*\x r) }); % clockwise double winding
			...
			{$\displaystyle
				f(z)=\frac{1}{z^{2}}=\sum_{n=-\infty}^{\infty} c_n z^n,\qquad
				c_n=\frac{1}{2\pi i}\!\oint_C \frac{f(\zeta)}{\zeta^{\,n+1}}\,d\zeta.$\\[4pt]
				For $f(z)=1/z^{2}$:\quad $c_{-2}=1\ (= \frac{1}{2\pi i}\!\oint_C \frac{1}{\zeta}\,d\zeta)$,\quad
				$c_n=0\ (n\neq -2).$\\[6pt]
