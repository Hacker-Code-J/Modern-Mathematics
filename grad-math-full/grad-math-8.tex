...
Let $X$ be a Banach space (or more generally, a complete normed vector space) over the field $F$. A sequence $\set{x_n}_{n=1}^\infty\subseteq X$ is called a \textbf{Schauder basis} for $X$ it if satisfies the following condition: \\
...
\defbox[Partial Order]{\begin{definition*}
	Let $S$ be a set. A binary relation $\preceq$ on $S$ (\ie, $\preceq\ \subseteq S\times S$) is called a \hl{\textbf{partial order}} if it satisfies the following three axioms for all $a,b,c\in X$, \begin{enumerate}[(i)]
...
\defbox[Chain]{\begin{definition*}
	Let $(P,\preceq)$ be a poset. A subset $C\subseteq P$ is called a \hl{\textbf{chain}} if \[
...
\end{tikzpicture}
\end{center}
Here, $C=\set{1,2,4,8,16}$ is a \textit{chain} under divisibility.
\end{example*}
\hfill
\begin{example*}
...
\end{tikzpicture}
\end{center}
Here, $C=\set{\varnothing,\set{a},\set{a,b},\set{a,b,c}}$ is a \textit{chain} in $2^S$.
\end{example*}

\newpage
\axiombox[Zorn's Lemma]{\begin{axiom*}
	Let $(P,\preceq)$ be a partially ordered set (poset) with property that every chain $C\subseteq P$ has an upper bound in $P$; that is, for every chain $C\subseteq P$, \[
