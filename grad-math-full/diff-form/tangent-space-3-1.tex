...
%	}
	...
	\addplot[-{Stealth}, domain=-2:5, samples=100, line width=.35mm, teal] {((x+1)*(x-2)*(x-2) + 1} node[right] {$\gamma(t)$};
%	\node[teal,above] at (-1,11) {$\gamma(a)$};
	%	\draw[fill=teal] (-1,11) circle (1.5pt);
%	\draw[fill=teal] (1.4,7.544) circle (2pt);
