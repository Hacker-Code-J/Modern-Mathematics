...
%	axis line style={opacity=0},
	...
	\node[above right, red] at (axis cs: 3, 3^3-3*3^2+5) {Curve $C\subseteq\mathbb{R}^2$};
...
%	\draw[dashed, cyan] (\a+1,\ay) -- (\a+1,\ay+\slope) node[midway,right] {$f'(a)$};
	...
	% --- Mark the point p ---
	\draw[dashed, magenta, align=left] (\a,\ay) to (\a,0) node[below] {$x(p)$ $=a$};
	\draw[dashed, magenta, align=left] (\a,\ay) to (0,\ay) node[left] {$y(p)$ $=f(a)$};
	\node[circle, fill=magenta!50, draw=black, inner sep=1.5pt, label={right:$\color{magenta}p=(x(p),y(p))$}] at (axis cs: \a, \ay) {};
