...
\begin{note}[Illustration]
	...
	% Draw the arrows representing the function
	\draw[-Stealth, thick] (-5.5, 4.25) -- (-.5,4.25) node[midway, above] {$f_1$};
	\draw[-Stealth, thick] (.5, 4.25) -- (5.5,4.25) node[midway, above] {$f_2$};
...
\begin{enumerate}[(1)]
	...
	\item \begin{itemize}
		...
		\begin{center}\begin{tikzpicture}[>=Stealth, scale=.6]
				...
				\draw[->, dotted, gray!40!white, line width = .25mm] (0, 2) -- (6, 0) node[midway] {?};
				\draw[->, dotted, gray!40!white, line width = .25mm] (0, 0) -- (6, 0) node[midway] {?};
				...
				\draw[->, dotted, red!40!white, line width = .25mm] (-6, 0) -- (-.5, 1) node[midway, above] {$g(b)$};
			\end{tikzpicture}
		\end{center}
		...
		\] for each $b\in B$. Thus, \begin{align*}
			...
			&=f(a) &\text{by def. of $g$} \\
			&=b &\text{by assumption}
		\end{align*} for all $b\in B$. Without the Axiom of Choice, we cannot always guarantee the existence of such a selection function, especially when the sets $f^{-1}[\set{b}]$ are uncountable.
		\vspace{12pt}
...
\begin{note}[Axiom of Choice]
	...
	\begin{tikzpicture}[>=Stealth, scale=.75]
		...
		\draw[->] (-3.5, 5.5) -- (3.5, 5.5) node[midway, above] {Choice Function $f$};
		
		\draw[thick] (-4.5,3.5) ellipse (2 and 1.25); \node at (-6.75, 3.5) {$A$};
		\draw[thick] (-4.5,.5) ellipse (2 and 1.25); \node at (-6.75, .5) {$B$};
		\draw[thick] (-4.5,-2.5) ellipse (2 and 1.25); \node at (-6.75, -2.5) {$C$};
		\node at (-4.5, -4.5) {$\bullet\ \bullet\ \bullet$};
