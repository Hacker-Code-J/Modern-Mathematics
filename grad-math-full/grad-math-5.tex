...
\lembox{\begin{lemma}
	\begin{enumerate}[(1)]
		\item The Intersection of Finite Sets is Finite or Empty
		
		Let \( \{A_i\}_{i \in I} \) be a collection of finite sets. Then:
		\[
		\bigcap_{i \in I} A_i
		\]
		is finite if \( I \) is finite, or empty otherwise.
		\item The Union of Finitely Many Finite Sets is Finite
		
		 Let \( \{A_i\}_{i=1}^n \) be a finite collection of finite sets. Then:
		\[
		\bigcup_{i=1}^n A_i
		\]
		is finite.
