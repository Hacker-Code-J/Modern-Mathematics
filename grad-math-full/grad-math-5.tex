...
\defbox[Open Epsilon-Ball]{\begin{definition*}
 Let $(S,d)$ be a metric space, where $S$ is a set and $d:S\times S\to\R$ is a metric. For $x\in S$ and $\varepsilon\in\R_{>0}$, the \textbf{open $\boldsymbol{\varepsilon}$-ball}\footnote{Open ball with center $x$ and radius $\varepsilon$} \textbf{of $x$ in $S$}, denoted by $B_\varepsilon(x)$, is defined as \[
...
\defbox[Neighborhood (Topology)]{\begin{definition*}
	...
	\begin{enumerate}[(1)]
		\item (Neighborhood of a Set) Let $A\subseteq S$. $\nbhd_A$ is a \textbf{neighborhood of $A$} if \[
		\exists U\in\tau\ \text{such that}\ A\subseteq U\subseteq\nbhd_A\subseteq S.
		\]
...
%\begin{tikzpicture}
%	% Topological space S
%	\draw[thick, rounded corners=20pt, blue!50, fill=blue!10] (0, 0) rectangle (8, 6) node[above left] {\Large $S$};
%	
%	% Neighborhood of a set (A)
%	\node[draw, thick, ellipse, fill=orange!20, label=below:{\Large $\nbhd_A$}] 
%	(neighborhoodA) at (3.5, 4.5) [minimum width=4.5cm, minimum height=2.5cm] {};
%	
%	% Open set containing A
%	\node[draw, thick, ellipse, fill=green!20, label=below:{\Large $U$}] 
%	(openSet) at (3.5, 4.5) [minimum width=3.5cm, minimum height=2cm] {};
%	
%	% Subset A
%	\node[draw, thick, ellipse, fill=red!30, label=below:{\Large $A$}] 
%	(subsetA) at (3.5, 4.5) [minimum width=2cm, minimum height=1cm] {};
%	
%	% Explanation for neighborhood of a set
%	\node at (3.5, 5.7) {\textbf{Neighborhood of a Set}};
%	\node[align=center] at (3.5, 3.2) 
%	{\small $A \subseteq U \subseteq \nbhd_A \subseteq S$};
%	
%	% Neighborhood of a point (a)
%	% Point a
%	\node[circle, draw=thick, blue, minimum size=6pt, fill=blue, label=below:{\Large $a$}] 
%	(point) at (6.5, 2) {};
%	
%	% Neighborhood of the point
%	\draw[thick, dashed, red!50] (6.5, 2) circle (1.5) node[below right] {\Large $\nbhd_a$};
%	
%	% Open set containing the point
%	\draw[thick, green!50, dotted] (6.5, 2) circle (1) node[above right] {\Large $U$};
%	
%	% Explanation for neighborhood of a point
%	\node at (6.5, 4) {\textbf{Neighborhood of a Point}};
%	\node[align=center] at (6.5, 0.5) 
%	{\small $a \in U \subseteq \nbhd_a \subseteq S$};
%	
%\end{tikzpicture}
%\end{center}
\newpage
...
\begin{tikzpicture}[scale=.85]
	% Draw the main set S
	\draw[thick, dashed, gray] (-6,-4.5) rectangle (10,4.25) node[black, above left] {Metric Space $(S,d)$};
...
\end{tikzpicture}
\begin{remark*}
	Let $(S,\tau)$ be a topological space. Then \[
	U\subseteq S\ \text{is \textbf{open} in $S$}\overset{\text{def}}{\iff}\forall p\in U,\exists V\in\tau\ \text{such that}\ p\in V\subseteq U.
	\]\begin{center}
	\begin{tikzpicture}[scale=.85]
		\draw[thick, dashed, gray] (-6,-4.5) rectangle (10,4.25) node[black, above left] {Topological Space $(S,\tau)$};
		
%		\fill[pattern=north west lines, pattern color=blue!10] (0,0) circle [radius=4];
		\node[right=3.5cm, blue] at (0,0) {$U$};
		\draw[blue, thick, dashed, smooth] plot coordinates {(-4.5,0) (-4.5,-2) (-3, -3) (-2, -4) (-1, -3.5) (1, -4) (3,-3.25) (4, -3) (5, -2) (5, -1) (4, 0) (4, 1) (3.5, 2) (3.5, 3) (2, 4) (0,3.5) (-1, 3.75) (-2, 4) (-3, 2) (-4, 1) (-4.5,0)};
		\fill[pattern=north west lines, pattern color=blue!10, smooth] plot coordinates {(-4.5,0) (-4.5,-2) (-3, -3) (-2, -4) (-1, -3.5) (1, -4) (3,-3.25) (4, -3) (5, -2) (5, -1) (4, 0) (4, 1) (3.5, 2) (3.5, 3) (2, 4) (0,3.5) (-1, 3.75) (-2, 4) (-3, 2) (-4, 1) (-4.5,0)};
		
		% Draw the epsilon-neighborhood around p
		\node[above=1.25cm] at (1,1) {\textcolor{green!50!black}{$\exists V_1\in\tau$}};
		\draw[green!50!black, thick, dashed, smooth] plot coordinates {(0,1) (.5,.8) (1,0) (2.5,1.5) (2.5,2.2) (2, 2.5) (1, 2) (.5, 2) (0,1)};
		\fill[pattern=north east lines, pattern color=green!50, smooth] plot coordinates {(0,1) (.5,.8) (1,0) (2.5,1.5) (2.5,2.2) (2, 2.5) (1, 2) (.5, 2) (0,1)};
		
		\node[right=.5cm] at (-3.5,0) {\textcolor{green!50!black}{$\exists V_2\in\tau$}};
		\draw[green!50!black, thick, dashed, smooth] plot coordinates {(-3.75,0) (-3.5,1) (-3.25,1) (-3,0) (-3,-1) (-2,-2) (-3,-2) (-3.5,-1) (-3.9, -.5) (-3.75,0)};
		\fill[pattern=north east lines, pattern color=green!50, smooth] plot coordinates {(-3.75,0) (-3.5,1) (-3.25,1) (-3,0) (-3,-1) (-2,-2) (-3,-2) (-3.5,-1) (-3.9, -.5) (-3.75,0)};
		
		\node[above=0.75cm] at (2,-2) {\textcolor{green!50!black}{$\exists V_3\in\tau$}};
		\draw[green!50!black, thick, dashed, smooth] plot coordinates {(1,-2) (1.5,-1.5) (1.5, -1) (3, -1.5) (2.5, -2) (2.75,-2.5) (2, -3) (1.5, -3.5) (1,-2)};
		\fill[pattern=north east lines, pattern color=green!50, smooth] plot coordinates {(1,-2) (1.5,-1.5) (1.5, -1) (3, -1.5) (2.5, -2) (2.75,-2.5) (2, -3) (1.5, -3.5) (1,-2)};
		...
		\node[] at (7,-4) {
			\begin{minipage}{7cm}
				\small
				\textbf{Every point $p \in U$ has an}\\ \textbf{open set $V\in\tau$ such that $p\in V\subseteq U$.}
			\end{minipage}
		};
	\end{tikzpicture}\end{center}
\end{remark*}
...
\begin{tikzpicture}[scale=.6]
	...
	\draw[dashed, opacity=.3] (0,\L+\epsilon) -- (\xMax+1,\L+\epsilon) node[right] {$L+\varepsilon$};
	\draw[dashed, opacity=.3] (0,\L-\epsilon) -- (\xMax+1,\L-\epsilon) node[right] {$L-\varepsilon$};
...
\begin{theorem*} 
	Consider a topological space $(\R,\tau_d)$, where \begin{align*}
		\tau_d:&=\set{U\subseteq\R:\forall p\in U,\ \exists\varepsilon>0\ \text{such that}\ \nbhd_\varepsilon(p)\subseteq U}.
	\end{align*} Let $\set{a_n}\subseteq\R$ and $\set{b_n}\subseteq\R$. Let $\lim\limits_{n\to\infty}a_n=\alpha\in\R$, $\lim\limits_{n\to\infty}b_n=\beta\in\R$, and $k\in\R$. Then 
	\begin{enumerate}[(1)]
...
\begin{enumerate}[(1)]
	\item \textcolor{blue}{Let $\varepsilon>0$}. Since $\lim\limits_{n\to\infty}a_n=\alpha$, we know \[
	\textcolor{blue}{\exists N\in\N}\ \text{s.t.}\ n\geq N\implies\abs{a_n-\alpha}<\textcolor{red}{\frac{\varepsilon}{\abs{k}+1}}
	\] Thus, \textcolor{blue}{if $n\geq N$ then} \begin{align*}
		...
		&<\abs{k}\cdot\textcolor{red}{\frac{\varepsilon}{\abs{k}+1}}\\
