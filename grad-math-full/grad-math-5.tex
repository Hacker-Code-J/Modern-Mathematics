...
%	\draw[thick, dashed, gray] (0,0) circle [radius=4] node[below left=3.5cm, black] {Metric Space};
	...
	\draw[dotted, thick, blue] (0,0) circle [radius=2];
	\node[above right] at (2,1.5) {\textcolor{blue}{$\nbhd_\varepsilon(L)$}};
	...
	\filldraw[blue] (-1,1) circle (2pt) node[right] {$a_{i}$};
	...
	\draw[blue, dashed, ->, thick] (-1.5,.5) -- (-1,1); %15-16
	\draw[blue, dashed, ->, thick] (-1,1) -- (-.5,1.5) node[right] {$\cdots$};
	
	% Annotation for sequence behavior
	\node[right=1cm] at (3.5,-3.5) {
		\begin{minipage}{10cm}
			\small
