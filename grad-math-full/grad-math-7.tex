...
Let \( S \) be a nonempty set. A \emph{binary operation} on \( S \) is a function
\[
\ast : S \times S \to S,
\]
which satisfies
\[
\forall\, a, b \in S,\quad \ast(a,b) \in S.
\]
...
An algebraic structure \((S,\ast)\) is called a \emph{semigroup} if and only if:
\begin{enumerate}[(i)]
	\item \( S \neq \varnothing \),
...
A \emph{monoid} is a semigroup \((S,\ast)\) that contains an \emph{identity element}. That is,
\[
...
A \emph{group} is a monoid \((S,\ast)\) in which every element has an \emph{inverse}. Formally,
\[
