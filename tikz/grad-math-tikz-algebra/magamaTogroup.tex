...
\begin{tikzpicture}[
	...
	% Nodes
	\node[block] (set) {Non-empty set};
	\node[block, right=of set] (magma) {Magma\\(binary operation)};
	\node[block, right=of magma] (semigroup) {Semigroup\\(associativity)};
	\node[block, right=of semigroup] (monoid) {Monoid\\(identity)};
	\node[block, right=of monoid] (group) {Group\\(inverses)};
	
	% Arrows with labels
	\draw[arrow] (set) -- node[above] {define $\cdot$} (magma);
	\draw[arrow] (magma) -- node[above] {associativity} (semigroup);
	\draw[arrow] (semigroup) -- node[above] {add identity} (monoid);
	\draw[arrow] (monoid) -- node[above] {add inverses} (group);
