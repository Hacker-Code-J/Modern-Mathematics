...
\begin{example*}
	...
	\begin{center}
	\input{../tikz/grad-math-tikz/grad-math-2-relation}
	\end{center}
\end{example*}
...
\begin{remark*}
	...
	\adjustbox{scale=.9}{\input{../tikz/grad-math-tikz/grad-math-2-equivalence-relations}}
	\end{figure}
\end{remark*}
\vfill
\begin{example*}
	...
	\begin{figure}[h!]\centering
		\input{../tikz/grad-math-tikz/grad-math-2-equivalence-relation-example}
	\end{figure}
\end{example*}
...
\probox[Equivalence Relation on $2^A$ Based on Bijection]{\begin{proposition}
	...
	X\sim_{\cR} Y\iff \exists f\in Y^X\ \text{such that $f$ is bijective},
	\] for $X,Y\in 2^A$. In other words, \[
	\mathcal{R}:=\set{(X,Y)\in 2^A\times 2^A:\exists\ \text{a bijection}\ f\in Y^X}.
	\] Then $\mathcal{R}$ is an equivalence relation on $2^A$.
\end{proposition}}
\begin{proof}
	Let $X,Y,Z\in 2^A$. We must show that ${\mathcal{R}}$ is reflexive, symmetric and transitive: 
	\begin{enumerate}[(i)]
...
\defbox[Indexed Family]{\begin{definition*}
	Let $I$ and $S$ are sets. Consider a function $A:I\to S$ defined by $i\mapsto A(i)=:A_i$. The image $\img{A}$ is called an \textbf{\hl{indexed family} of elements in $S$ indexed by $I$}. We write this indexed family as: $\Gen{A_i}_{i\in I}$. Note that \[
...
%	\begin{center}
%	\input{../tikz/grad-math-tikz/grad-math-2-indexed-family}
%	\end{center}
\end{example*}
\vfill
\defbox[Union and Intersection of an Indexed Family]{\begin{definition*}
	Let $I$ and $S$ are sets, and let $\Gen{A_i}_{i\in I}$ be an indexed family in $S$. \begin{itemize}
		\item The \textbf{union of} $\Gen{A_i}_{i\in I}$ is defined by $
		\displaystyle\boxed{\bigcup_{i\in I}A_i:=\set{x\in S:\exists i\in I\ \text{such that}\ x\in A_i}}$. \begin{center}
			\begin{tikzpicture}[scale=.55]
				\draw[rotate=-45, thick, fill=blue, fill opacity=0.2] (-1.5,0) ellipse (2cm and 1.25cm);
				\draw[rotate=45, thick, fill=red, fill opacity=0.2] (1.5,0) ellipse (2cm and 1.25cm);
				\draw[rotate=-45, thick, fill=green, fill opacity=0.2] (-1.5,1.4) ellipse (2cm and 1.25cm);
				\draw[rotate=45, thick, fill=yellow, fill opacity=0.2] (1.5,1.4) ellipse (2cm and 1.25cm);
				\foreach \points in {(-1,0)}
				\fill[red] \points circle (3pt) node[right] {$x\in A_i$ for some $i\in I$};
			\end{tikzpicture}
		\end{center}
		\item The \textbf{intersection of} $\Gen{A_i}_{i\in I}$ is defined by $
		\displaystyle\boxed{\bigcap_{i\in I}A_i:=\set{x\in S:\forall i\in I,\ x\in A_i}}$. 
		\begin{center}
			\begin{tikzpicture}[scale=.55]
				\draw[rotate=-45, thick, fill=blue, fill opacity=0.2] (-1.5,0) ellipse (2cm and 1.25cm);
				\draw[rotate=45, thick, fill=red, fill opacity=0.2] (1.5,0) ellipse (2cm and 1.25cm);
				\draw[rotate=-45, thick, fill=green, fill opacity=0.2] (-1.5,1.4) ellipse (2cm and 1.25cm);
				\draw[rotate=45, thick, fill=yellow, fill opacity=0.2] (1.5,1.4) ellipse (2cm and 1.25cm);
				\foreach \points in {(0,1.25)}
				\fill[red] \points circle (3pt) node[right] {$x\in A_i$ for every $i\in I$};
			\end{tikzpicture}
	\end{center}
	\end{itemize}
\end{definition*}}
...
\defbox[$\star$ Partitions $\star$]{\begin{definition*}
	Let $S$ be a set\textcolor{gray!20}{, and let the function $A:I\to 2^S$ as $i\mapsto A_i:=A(i)\subseteq S$, for all $i\in I$}. Consider a family of subsets $\gen{A_i}_{i\in I}$, where $A_i\subseteq S$ for every index $i\in I$. The family $\Gen{A_i}_{i\in I}$ is called a \textbf{\hl{partition} of $S$} if the following conditions are satisfied:
	\begin{enumerate}[(i)]
		\item (\textbf{Non-empty Subsets}) Each subset $A_i$ is non-empty, \ie, $
		\boxed{\forall i\in I,\ A_i\neq\varnothing}.$
		\item (\textbf{Pairwise disjoint}) For all distinct \( i, j \in I \), the subsets \( A_i \) and \( A_j \) are disjoint, i.e.,
		\[\boxed{\forall i, j \in I, \ i \neq j \implies A_i \cap A_j = \varnothing}.\]
		\item (\textbf{Union covers the entire set}) The union of all subsets \( A_i \) covers the whole set \( S \), i.e.,
		\[\boxed{\bigcup_{i \in I} A_i = S}.\]
...
\end{minipage}
\begin{minipage}{.5\textwidth}
\adjustbox{scale=.65,center}{\input{../tikz/grad-math-tikz/grad-math-2-partition}}
\end{minipage}
\end{example*}
...
\end{note}
\begin{center}
\input{../tikz/grad-math-tikz/grad-math-2-equiv-class}
\end{center}
\lembox[]{\begin{lemma}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\thmbox[$\star\star\star$ Fundamental Theorem on Equivalence Relations $\star\star\star$]{\begin{theorem}
	...
	\] Then $\mathcal{P}$ forms a partition of $S$.
\end{theorem}}
\begin{center}
\input{../tikz/grad-math-tikz/grad-math-2-FundThm-Equiv}
\end{center}
...
%\begin{tikzpicture}
%	
%	% Draw the five ellipses (for 5 sets)
%	\draw[rotate=0, thick, fill=blue, fill opacity=0.2] (0, 0) ellipse (2cm and 1cm); % First set
%	\draw[rotate=72, thick, fill=red, fill opacity=0.2] (0, 0) ellipse (2cm and 1cm); % Second set
%	\draw[rotate=144, thick, fill=green, fill opacity=0.2] (0, 0) ellipse (2cm and 1cm); % Third set
%	\draw[rotate=216, thick, fill=yellow, fill opacity=0.2] (0, 0) ellipse (2cm and 1cm); % Fourth set
%	\draw[rotate=288, thick, fill=purple, fill opacity=0.2] (0, 0) ellipse (2cm and 1cm); % Fifth set
%	
...
%\end{tikzpicture}
\newpage
\thmbox[$\star$ Relation Induced by Partition is Equivalence $\star$]{\begin{theorem}
...
%\newpage
%\appendix
%\section*{Several Relations}
%\begin{table}[h]\centering\setstretch{1.5}
%\resizebox{\textwidth}{!}{\begin{tabular}{|c|c|c|l|l|}\hline
%	\textbf{Reflexive} & \textbf{Symmetric} & \textbf{Transitive} & \textbf{Example1} & \textbf{Example 2} \\ \hline
%	\yes               & \yes               & \yes                & \( \cR = \{(a, b) \mid a = b\} \)              & \( \cR = \{(a, b) \mid a \equiv b \mod n\} \)     \\ \hline
%	\yes               & \yes               & \no                 & \( \cR = \mathbb{R}^2 \)                       & \( \cR = \{(a, b) \mid a \leq b^2\} \)            \\ \hline
%	\yes               & \no                & \yes                & \( \cR = \{(A, B) \mid A \subseteq B\} \)      & \( \cR = \{(a, b) \mid a \mid b\} \)              \\ \hline
%	\yes               & \no                & \no                 & \( \cR = \{(a, b) \mid a \leq b + 3\} \)       & \( \cR = \{(x, y) \mid f(x) \leq f(y)\} \)        \\ \hline
%	\no                & \yes               & \yes                & \( \cR = \{(a, b) \mid a - b \in 2\mathbb{Z}\} \) & \( \cR = \{(x, y) \mid |x| = |y|\} \)         \\ \hline
%	\no                & \yes               & \no                 & \( \cR = \{(a, b) \mid a, b \in \{1,2\}\} \)   & \( \cR = \{(a, b) \mid a \neq b\} \)              \\ \hline
%	\no                & \no                & \yes                & \( \cR = \{(a, b) \mid a^2 \leq b^3\} \)       & \( \cR = \{(x, y) \mid e^x \leq e^y\} \)          \\ \hline
%	\no                & \no                & \no                 & \( \cR = \{(a, b) \mid a = 2, b \neq 2\} \)    & \( \cR = \{(x, y) \mid x \text{ is prime, } y \text{ is composite}\} \) \\ \hline
%\end{tabular}}
%\caption{Classification of Relations}
%\end{table}
