...
\begin{tikzpicture}[
	...
	\node[scale=1.2] (eq) at (0, 0) {
			$\chi_A(\lambda) = -\lambda^3 + \textcolor{blue}{c_2} \lambda^2 - \textcolor{orange}{c_1} \lambda + \textcolor{green!60!black}{c_0}$
		};
	
	% --- 2. Coefficient Visualizations (Branches) ---
	
	% === Branch 1: Trace (c2) ===
	\node[below=2cm of eq, xshift=-5.5cm] (trace_node) {};
...
%	\node[below=0mm of lbl_trace, scale=0.8] {$a + e + i$};
	...
	\node[below=0.3cm of M_trace, scale=0.8] {$a + e + i$};
	...
	% Minor {1,2} (Top Left)
	\matrix[mtx, scale=0.8] (M_m1) at ($(minor_node)+(-2.5, 0)$) {
			a \& b \& c \\
			d \& e \& f \\
			g \& h \& i \\
		};
	\begin{scope}[on background layer]
		...
		\end{scope}
	\node[below=0.1cm of M_m1, scale=0.7] {$\det\begin{bmatrix} a&b \\ d&e\end{bmatrix}$};
	...
	\begin{scope}[on background layer]
		...
		\end{scope}
	\node[below=0.1cm of M_m2, scale=0.7] {$\det\begin{bmatrix} e&f \\ h&i\end{bmatrix}$};
	
	% Minor {1,3} (Corners) - The tricky one
	\matrix[mtx, scale=0.8] (M_m3) at ($(minor_node)+(2.5, 0)$) {
			a \& b \& c \\
			d \& e \& f \\
			g \& h \& i \\
		};
	\begin{scope}[on background layer]
		...
		\end{scope}
	\node[below=0.1cm of M_m3, scale=0.7] {$\det\begin{bmatrix} a&c \\ g&i\end{bmatrix}$};
	
%	\node[below=1.2cm of minor_node, term label, text=orange] (lbl_minor) {Sum of 2-Minors};
	
	
	% === Branch 3: Determinant (c0) ===
	\node[below=2cm of eq, xshift=5.5cm] (det_node) {};
...
%	\node[below=0mm of lbl_det, scale=0.8] {$\det(A)$};
	...
	\node[below=0.3cm of M_det, scale=0.8] {$\det(A)$};
	
	% --- 3. Arrows and Annotations ---
	
	% Arrow to Trace
	\draw[arrow, blue] ($(eq.south)+(.25,0)$) to[out=270, in=90] node[midway, font=\scriptsize] {} (M_trace.north);
	
	% Arrow to Minors
	\draw[arrow, orange] ($(eq.south)+(1.5,0)$) to[out=270, in=90] node[midway, inner sep=1pt, font=\scriptsize] {} (M_m2.north);
	
	% Arrow to Det
	\draw[arrow, green!60!black] ($(eq.south)+(2.75,0)$) to[out=270, in=90] node[midway, font=\scriptsize] {} (M_det.north);
	
	% Plus signs between the minors
	\node at ($(M_m1)!0.5!(M_m2)+(0,-1.325)$) {$+$};
	\node at ($(M_m2)!0.5!(M_m3)+(0,-1.325)$) {$+$};
\end{tikzpicture}
