...
Let $A=(a_{ij})\in\Mat_{n}(\F)$ and $\set{i_1<i_2<\cdots<i_\ell}\subset\set{1,\dots,n}$. For each $\ell=1,\dots,n$, we define \[
...
\begin{theorem}
Let $A=(a_{ij})\in\Mat_n(\F)$. Define the characteristic polynomial \[
\chi_A(\lambda)\coloneqq \det(A-\lambda I_n)=\sum_{i=0}^{n}c_i\lambda^i\in \F[\lambda].
\]
Then $\chi_A(\lambda)$ is 
\[
\chi_A(\lambda)=(-1)^n\left(\lambda^n-E_1(A)\lambda^{n-1}+E_2(A)\lambda^{n-2}-\cdots+(-1)^nE_n(A)\right).
\]
