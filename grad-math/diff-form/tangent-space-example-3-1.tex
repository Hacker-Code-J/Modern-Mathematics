...
\begin{axis}[
	...
	\addplot[domain=-1:1.4, samples=100, thick, red] {x^2};
...
%	\draw[dashed, cyan] (\a,\ay) to (0,\ay) node[left] {$1^2$};
	...
	% --- Mark the point p ---
	\draw[dashed, magenta] (\a,\ay) to (\a,0) node[below] {$x(p)=1$};
	\draw[dashed, magenta] (\a,\ay) to (0,\ay) node[left] {$y(p)=1^2$};
	\draw[fill=magenta!50] (\a, \ay) circle (2pt) node[right=5pt, magenta, align=left] {$p=(x(p),y(p))$\\$=(1, 1^2)$};
	\draw[fill=magenta!50] (0,1) circle (1.5pt);
	\draw[fill=magenta!50] (1,0) circle (1.5pt);
